...
EMBEDDING_MODEL_CONFIG__ENCODING_FORMAT=base64  # base64, float, or empty to omit
...
`EMBEDDING_MODEL_CONFIG__ENCODING_FORMAT=float` is useful for OpenAI-compatible
providers such as OpenRouter's Gemini embeddings, which reject base64 embedding
payloads. Leave it empty to omit `encoding_format` and use the provider default.
`EMBEDDING_BASE_URL` is a shorthand for `EMBEDDING_MODEL_CONFIG__OVERRIDES__BASE_URL`;
the nested value wins if both are set.
